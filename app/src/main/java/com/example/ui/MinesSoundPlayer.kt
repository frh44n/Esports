...
object MinesSoundPlayer {
    fun playGemSound() {
        playTone(listOf(523.25, 659.25, 783.99), 150) // C5, E5, G5 major triad quick chime
    }

    fun playExplodeSound() {
        playNoise(400) // explosion noise / low frequency rumbling
    }
    ...
    private fun playNoise(durationMs: Int) {
        Thread {
            try {
                ...
                for (i in 0 until numSamples) {
                    val dVal = (random.nextDouble() * 2.0 - 1.0)
                    val progress = i.toDouble() / numSamples
                    val envelope = (1.0 - progress) * (1.0 - progress)
                    val valShort = (dVal * 32767 * envelope).toInt().toShort()
                    generatedSnd[idx++] = (valShort.toInt() and 0x00ff).toByte()
                    generatedSnd[idx++] = ((valShort.toInt() and 0xff00) ushr 8).toByte()
                }
