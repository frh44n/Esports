...
target = """    val inAppNotifications = MutableStateFlow<List<InAppNotification>>(emptyList())
}"""

replacement = """    val inAppNotifications = MutableStateFlow<List<InAppNotification>>(emptyList())
    ...
    fun requestLudoMatch(user: com.example.data.User, tournament: com.example.data.Tournament) {
        ...
        ludoMatchRequests.value = ludoMatchRequests.value + req
    }
